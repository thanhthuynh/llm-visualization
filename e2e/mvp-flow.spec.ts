...
test('keyboard ArrowDown advances through the pipeline to About', async ({ page }) => {
  ...
  // 5 ArrowDowns to get from prompt → tokenize → predict → decode → output → about
  for (let i = 0; i < 5; i++) {
...
test('rail jump goes directly to Tokenization', async ({ page }) => {
  ...
  await page.getByRole('button', { name: /tokenization/i }).first().click()
  await expect(page).toHaveURL(/#tokenize$/)
  await expect(page.getByRole('heading', { level: 2, name: 'Tokenization' })).toBeVisible()
})

test('rail navigation between scenes updates URL hash and active rail segment', async ({ page }) => {
  await page.goto('/')
  await page.getByRole('button', { name: /decoding loop/i }).first().click()
