...
export function TopBar({ scene }: TopBarProps) {
  ...
  return (
    <header className="fixed top-0 right-0 left-[72px] h-18 bg-bg-base border-b border-border flex items-center justify-between px-4 sm:px-8 z-[9]">
      <span className="font-display font-bold text-xl tracking-[-0.2px]">Inside an LLM</span>
      {!isPrologue && (
        <div
          data-testid="topbar-pill"
          className="hidden sm:inline-flex items-center gap-2.5 px-3.5 py-1.5 rounded-pill bg-surface-card border border-border max-w-[55vw] overflow-hidden"
        >
          {accentColor ? (
