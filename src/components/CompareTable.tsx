...
export function CompareTable({ rows, caption }: CompareTableProps) {
  return (
    ...
    >
      {caption && (
        <caption className="text-left text-text-muted text-[11px] pb-2">{caption}</caption>
      )}
      <tbody>
        {rows.map((r, i) => (
          <tr key={`${r.label}-${i}`} className="border-t border-border">
            <th scope="row" className="text-left px-2.5 py-2 font-semibold text-text-primary">
              {r.label}
            </th>
